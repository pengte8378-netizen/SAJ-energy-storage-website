...
function calculate(req: any, preferred?: Product) {
  ...
  const requested: Product = preferred ?? (req.scenario === "storage" || req.scenario === "vpp" || req.power > 500 ? "CM2" : req.power > 100 || req.pv > 200 ? "CHS3" : "CHS2");
  const product: Product = backupRestricted && requested === "CM2" ? (req.power > 100 || req.pv > 200 ? "CHS3" : "CHS2") : requested;
...
export default function Home() {
  ...
  useEffect(() => {
    const stored = window.localStorage.getItem("saj-content-assets-v2");
    if (stored) {
  ...
  useEffect(() => {
    window.localStorage.setItem("saj-content-assets-v2", JSON.stringify(contentAssets));
  }, [contentAssets]);
