...
const products = {
  CHS2: { title: "CHS2 混合储能系统", image: "/products/chs2.webp", power: 50, capacity: 100.3, tags: ["≤200 kWh 优先推荐", "单机最多 4 个电池柜", "内置 STS"] },
...
const europeanCountries = new Set(["意大利", "德国", "法国", "西班牙", "葡萄牙", "荷兰", "比利时", "奥地利", "波兰", "瑞典", "芬兰", "丹麦", "挪威", "瑞士", "捷克", "罗马尼亚", "斯洛文尼亚", "爱尔兰", "英国"]);

function ch3Variant(country: string) {
  if (country === "澳大利亚") return { code: "CH3-125K-T8-AU", label: "CH3-AU" };
  if (europeanCountries.has(country)) return { code: "CH3-125K-T8-EU", label: "CH3-EU" };
  return { code: "CH3-125K-T8", label: "CH3" };
}

function voltageValue(value: string) {
  return Number(value.match(/\d+/)?.[0] || 400);
}
...
function calculate(req: any, preferred?: Product, country = "中国") {
  const backupRestricted = req.scenario === "backup" || req.backup;
  const capacityRecommended: Product = req.capacity <= 200 ? "CHS2" : "CHS3";
  const requested: Product = preferred ?? capacityRecommended;
  const product: Product = backupRestricted && requested === "CM2" ? capacityRecommended : requested;
  const p = products[product];
  const capacityCabinets = Math.max(1, Math.ceil(req.capacity / p.capacity));
  const powerInverters = Math.max(1, Math.ceil(req.power / p.power));
  const inverterQty = product === "CM2" ? Math.max(powerInverters, capacityCabinets) : Math.max(powerInverters, Math.ceil(capacityCabinets / 4));
  const batteryQty = product === "CM2" ? inverterQty : Math.max(capacityCabinets, inverterQty);
  const variant = ch3Variant(country);
  const modelCode = product === "CHS2" ? "CHS2-50K-T6" : product === "CHS3" ? variant.code : "CM2-125K-261";
  const productLabel = product === "CHS3" ? variant.label : product;
  const bom: Bom[] = [{ code: modelCode, name: product === "CHS3" ? `${variant.label} 液冷混合逆变器` : p.title, type: product === "CM2" ? "储能主机" : "混合逆变器", qty: inverterQty, reason: `按 ${req.power} kW 功率目标及单机最多连接 4 个电池柜计算` }];
  if (product === "CHS2") bom.push({ code: "CB2-100.3K-HV5", name: "CB2 高压电池柜", type: "电池系统", qty: batteryQty, reason: `满足 ${req.capacity} kWh 目标；每台逆变器连接 1–4 柜` });
  if (product === "CHS3") bom.push({ code: "CB3-L261", name: "CB3 液冷电池柜", type: "电池系统", qty: batteryQty, reason: `满足 ${req.capacity} kWh 目标；每台逆变器连接 1–4 柜` });
  if (product === "CM2") bom.push({ code: "CM2-EMS", name: "CM2 能量管理控制器（内置电表）", type: "控制器 / 电网计量", qty: 1, reason: "单台或多台 CM2 均默认必配，使用内置电表" });
  if (product === "CHS2" && inverterQty > 1) bom.push({ code: "eManager-C1 Pro", name: "系统能量管理控制器（内置电表）", type: "控制器 / 电网计量", qty: 1, reason: "CHS2 多机配置，使用 eManager-C1 Pro 内置电表" });
  if (product === "CHS2" && inverterQty === 1) bom.push({ code: "DTSU666", name: "电网电表", type: "电网计量", qty: 1, reason: "单台 CHS2 必配" });
  if (product === "CHS3") bom.push({ code: "EM53ER REV.A(1B1T,5A)", name: "电网电表", type: "电网计量", qty: 1, reason: "单台或多台 CH3 均配置 1 台" });
  ...
  const pvCtRating = selectCtRating(pvCurrent);
  if (pvMeterEnabled) {
...
export default function Home() {
  ...
  const [req, setReq] = useState({ scenario: "new" as Scenario, coupling: "DC" as "AC" | "DC", existingGridPv: false, pvMeter: false, pvPowerFactor: 1, power: 250, capacity: 522, pv: 420, voltage: "400V / 50Hz", transformer: 800, gridExport: false, backup: true, blackStart: false, dg: false, vpp: false, environment: "outdoor", minTemp: -10, maxTemp: 42, altitude: 320 });
  const [result, setResult] = useState(() => calculate(req, undefined, project.country));
  const [bom, setBom] = useState<Bom[]>(() => calculate(req, undefined, project.country).bom);
  ...
  useEffect(() => {
    const stored = window.localStorage.getItem("saj-content-assets-v3");
    if (stored) {
  ...
  useEffect(() => {
    window.localStorage.setItem("saj-content-assets-v3", JSON.stringify(contentAssets));
  }, [contentAssets]);
  ...
  useEffect(() => {
    if (backupRestricted && preferredProduct === "CM2") {
      setPreferredProduct("AUTO");
      setToast("备电或离网模式不支持 CM2，已恢复为智能推荐");
    }
  }, [backupRestricted, preferredProduct]);
  ...
  async function exportPdf() {
    ...
    const selected = contentAssets.filter((asset) => modules.includes(asset.moduleId || asset.id) && asset.status === "published" && (asset.product === "ALL" || asset.product === result.product));
    const moduleText = (id: string) => ({
      company: "SAJ 专注于智慧能源转换、储能及能源管理，为全球工商业客户提供产品、平台和全生命周期服务。",
      selling: `${result.productLabel} 核心优势：${product.tags.join("、")}。`,
      ...
      spec: `${result.productLabel} 规格书将作为附件，涵盖功率、容量、环境、尺寸及认证信息。`,
    ...
    host.innerHTML = `
      ...
      ${selected.map((asset, index) => renderContentAssetPage(asset, { product: result.product, productLabel: result.productLabel, productImage: product.image, power: result.power, capacity: result.capacity, inverterQty: result.inverterQty, batteryQty: result.batteryQty, country: project.country, industry: project.industry, backup: req.backup, pvMeterEnabled: result.pvMeterEnabled, pvCtRating: result.pvCtRating }, 3 + index, 3 + selected.length)).join("")}
    ...
    <section className="main-column">
      ...
      <div className="workspace"><section className="content">
        ...
        {step === 2 && <>
          ...
          <Panel title="04 · 电网、耦合与计量要求"><div className="compact-grid">
            <Field label="并网电压 / 频率"><select value={req.voltage} onChange={(e) => setReq({ ...req, voltage: e.target.value })}><option>400V / 50Hz</option><option>380V / 50Hz</option><option>415V / 50Hz</option></select></Field>
            <Field label="变压器额定容量"><input type="number" value={req.transformer} onChange={(e) => setReq({ ...req, transformer: Number(e.target.value) })} /></Field>
            <Field label="系统耦合方式"><select value={req.coupling} onChange={(e) => setReq({ ...req, coupling: e.target.value as "AC" | "DC", existingGridPv: e.target.value === "AC" ? req.existingGridPv : false, pvMeter: e.target.value === "AC" ? req.pvMeter : false })}><option value="DC">DC 耦合</option><option value="AC">AC 耦合</option></select></Field>
            <Toggle label="允许向电网反送电" on={req.gridExport} set={(value: boolean) => setReq({ ...req, gridExport: value })} />
            <Toggle label="要求并离网运行" on={req.backup} set={(value: boolean) => setReq({ ...req, backup: value })} />
            <Toggle label="要求黑启动" on={req.blackStart} set={(value: boolean) => setReq({ ...req, blackStart: value })} />
            <Toggle label="接入柴油发电机" on={req.dg} set={(value: boolean) => setReq({ ...req, dg: value })} />
            <Toggle label="参与 VPP 调度" on={req.vpp} set={(value: boolean) => setReq({ ...req, vpp: value })} />
            {req.coupling === "AC" && <Toggle label="已有并网光伏系统" on={req.existingGridPv} set={(value: boolean) => setReq({ ...req, existingGridPv: value, pvMeter: value ? req.pvMeter : false })} />}
            {req.coupling === "AC" && req.existingGridPv && <Toggle label="增加 PV Meter 计量" on={req.pvMeter} set={(value: boolean) => setReq({ ...req, pvMeter: value })} />}
            {req.coupling === "AC" && req.existingGridPv && req.pvMeter && <Field label="光伏系统功率因数"><input type="number" min="0.8" max="1" step="0.01" value={req.pvPowerFactor} onChange={(e) => setReq({ ...req, pvPowerFactor: Number(e.target.value) || 1 })} /></Field>}
          </div></Panel>
          <Panel title="05 · 安装环境"><div className="compact-grid env"><Field label="安装位置"><select value={req.environment} onChange={(e) => setReq({ ...req, environment: e.target.value })}><option value="outdoor">室外</option><option value="indoor">室内机房</option></select></Field><Field label="最低温度 (°C)"><input type="number" value={req.minTemp} onChange={(e) => setReq({ ...req, minTemp: Number(e.target.value) })} /></Field><Field label="最高温度 (°C)"><input type="number" value={req.maxTemp} onChange={(e) => setReq({ ...req, maxTemp: Number(e.target.value) })} /></Field><Field label="海拔 (m)"><input type="number" value={req.altitude} onChange={(e) => setReq({ ...req, altitude: Number(e.target.value) })} /></Field></div></Panel>
          <Actions><button className="secondary" onClick={() => setStep(1)}>← 返回</button><button className="primary" onClick={runRecommendation}>智能生成推荐配置 ✦</button></Actions>
        </>}

        {step === 3 && <><Heading n="03" title="智能推荐配置" sub="系统已基于项目参数、产品兼容关系和单机最多连接 4 个电池柜规则完成匹配。" badge="✓ 推荐完成" success /><div className="recommend"><div><img src={product.image} alt={product.title} /></div><section><span>最佳匹配 · {result.productLabel}</span><h2>{result.product === "CHS3" ? `${result.productLabel} 液冷混合储能系统` : product.title}</h2><p>{req.scenario === "new" ? "直流耦合光储架构" : "工商业储能架构"}</p><div className="result-metrics"><b>{result.power}<small>kW 额定功率</small></b><b>{result.capacity}<small>kWh 额定容量</small></b><b>{(result.capacity / result.power).toFixed(1)}<small>h 预计时长</small></b></div><div className="tags">{product.tags.map((tag) => <i key={tag}>{tag}</i>)}</div></section></div><Panel title="推荐设备清单" note="按规则自动计算"><div className="bom"><header><span>设备 / 型号</span><span>类型</span><span>数量</span><span>推荐依据</span></header>{bom.map((item, index) => <div key={`${item.code}-${index}`}><span><b>{item.code}</b><small>{item.name}</small></span><span><em>{item.type}</em></span><span><b>{item.qty}</b></span><span>{item.reason}</span></div>)}</div></Panel><div className="two-col"><Panel title={`为什么推荐 ${result.productLabel}？`}><ul className="reasons"><li>目标容量 {req.capacity} kWh，按“≤200 kWh 推荐 CHS2，＞200 kWh 推荐 CHS3”规则匹配。</li><li>功率目标 {req.power} kW，需要 {result.inverterQty} 台逆变器，配置达到 {result.power} kW。</li><li>{result.product === "CM2" ? "CM2 为电池一体化系统。" : `配置 ${result.batteryQty} 个电池柜，每台逆变器连接 1–4 个电池柜。`}</li><li>{result.product === "CHS3" ? `${project.country} 项目使用 ${result.productLabel} 区域版本，EMS 已内置。` : result.product === "CM2" ? "CM2 默认配置 1 台 CM2-EMS。" : result.inverterQty > 1 ? "CHS2 多机配置 1 台 eManager-C1 Pro。" : "单台 CHS2 无需单独配置 eManager-C1 Pro。"}</li></ul></Panel><Panel title="可选替代方案"><div className="alternative"><i>{backupRestricted ? "场景硬性限制" : "兼容替代方案"}</i><b>{backupRestricted ? "CM2 不可选" : result.product === "CM2" ? "CHS3 多机并联" : "CM2 交流耦合"}</b><small>{backupRestricted ? "备电或离网要求下仅支持 CHS2 / CHS3" : "无备电或离网要求时可切换兼容产品"}</small><button className="secondary full" disabled={backupRestricted} onClick={() => { const target: Product = result.product === "CM2" ? "CHS3" : "CM2"; const changed = calculate(req, target, project.country); setPreferredProduct(target); setResult(changed); setBom(changed.bom); setToast(`已切换为 ${changed.productLabel}`); }}>{backupRestricted ? "当前场景不可切换" : "应用此方案"}</button></div></Panel></div><Actions><button className="secondary" onClick={() => setStep(2)}>← 修改需求</button><button className="primary" onClick={() => next(4)}>检查配置安全性 →</button></Actions></>}
        ...
        {step === 5 && <><Heading n="05" title="构建技术方案" sub="必选内容已固定，可按项目需要添加、移除并调整可选模块。" badge={`预计 ${12 + modules.length * 2} 页`} /><div className="builder"><Panel title="方案目录" note="自动匹配内容"><h3 className="group-title">必选模块</h3>{["封面与项目摘要", "客户需求概述", "推荐系统架构与拓扑", "BOM 设备清单", "关键技术参数", "风险边界与免责声明"].map((title, index) => <div className="required" key={title}><i>{String(index + 1).padStart(2, "0")}</i><b>{title}</b><em>必选</em></div>)}<h3 className="group-title">可选模块</h3>{optionalModules.map(([id, fallbackTitle, fallbackSub], index) => {
          const asset = contentAssets.find((item) => (item.moduleId || item.id) === id && (item.product === "ALL" || item.product === result.product));
