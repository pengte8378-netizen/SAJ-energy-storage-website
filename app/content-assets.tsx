...
export const defaultContentAssets: ContentAsset[] = [
  ...
  { id: "selling-CHS2", moduleId: "selling", product: "CHS2", title: "CHS2 产品核心卖点", category: "产品内容", tags: "CHS2, DC耦合, STS", summary: "≤200 kWh 优先推荐、内置 STS 与单机多电池柜灵活扩展。", body: "CHS2 集成光伏逆变、储能变流与静态切换功能，适用于超市、农场、小型工厂和微电网。\n50 kW 机型最高可接入 100 kWp 光伏，支持 6 路 MPPT，并可在给负载供电的同时为电池充电。\n每台逆变器可连接 1–4 个 CB2 电池柜；多于 1 台 CHS2 时配置 1 台 eManager-C1 Pro。内置 STS 支持小于 20 ms 的并离网切换。", image: "/products/chs2.webp", status: "published", updated: "2026-08-11" },
  ...
  { id: "selling-CHS3", moduleId: "selling", product: "CHS3", title: "CHS3 产品核心卖点", category: "产品内容", tags: "CHS3, 1500V, DC-BUS", summary: "＞200 kWh 优先推荐、区域版本匹配、直流母线耦合与内置 EMS/STS。", body: "CHS3 根据项目地匹配版本：欧洲使用 CH3-EU，澳大利亚使用 CH3-AU，其他地区使用 CH3 标准版。\n每台 CH3 逆变器可连接 1–4 个 CB3-L261 液冷电池柜，支持不同储能时长和市场需求。\n系统内置 EMS 与 STS，不配置独立 EMS 控制器，并支持小于 20 ms 的并离网切换和多机并联。", image: "/products/chs3.webp", status: "published", updated: "2026-08-11" },
  ...
  { id: "spec-CHS3", moduleId: "spec", product: "CHS3", title: "CHS3 产品规格书", category: "技术资料", tags: "CHS3, 参数, 规格", summary: "CH3 75-125 kW 与 CB3-L261 液冷电池关键参数。", body: "逆变器额定交流功率覆盖 75-125 kW，最大光伏阵列功率 150-250 kWp。\nCB3-L261 采用 LFP 3.2V/314Ah 电芯，单柜额定能量 261 kWh；每台 CH3 可连接 1–4 个电池柜。\n系统最大效率不低于 98%，防护等级 IP66；欧洲、澳大利亚和其他地区分别匹配 CH3-EU、CH3-AU 与标准版本。", image: "/products/chs3.webp", status: "published", updated: "2026-08-11" },
...
export function renderContentAssetPage(asset: ContentAsset, context: { product: string; productLabel?: string; productImage: string; power: number; capacity: number; inverterQty: number; batteryQty: number; country: string; industry: string; backup: boolean; pvMeterEnabled: boolean; pvCtRating: number | null }, page: number, total: number) {
  ...
  const productFacts: Record<string, Record<string, string[]>> = {
    CHS2: {
      selling: [`${context.productLabel || context.product} · ${context.power} kW`, `${context.capacity} kWh 系统容量`, "单机连接 1–4 个电池柜", "多机配置 eManager-C1 Pro"],
    ...
    CHS3: {
      selling: [`${context.productLabel || context.product} · ${context.power} kW`, `${context.capacity} kWh 系统容量`, "单机连接 1–4 个 CB3-L261", "内置 EMS / STS"],
  ...
  const emsBody = context.product === "CM2"
    ? "单台或多台 CM2 均配置 1 台 CM2-EMS，并直接使用 CM2-EMS 内置电表。"
    : context.product === "CHS3"
      ? `${context.productLabel || "CHS3"} 内置 EMS，不需要独立 EMS 控制器；单台或多台逆变器均配置 1 台 EM53ER REV.A(1B1T,5A) 电网电表。`
